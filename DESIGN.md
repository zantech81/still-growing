...
**Content width:** `max-w-3xl` (768px) is the standard width across the entire reader app (Library, Journey, the Journey book-switcher, Circle, Growing, Account, Account's review form, the chapter/milestone claim page, the public profile page `/u/[userId]`, and the homepage). `/login` is a deliberate exception at `max-w-sm`, since it's a minimal single-field auth form, not a content page. Circle's reflection quotes (and the public profile's pinned-reflection cards) run longer than the 65–75ch prose-readability guideline above as a result — accepted for site-wide visual consistency. Not part of this convention: The Grove, `/privacy`, `/reviews`, and `/r/[shareId]` stay at `max-w-2xl`, since they're long-form prose, a review feed with the same measure tradeoff as Circle, or an already-documented cold-traffic exception — not yet reconciled against this rollout.
...
**Sprout** (`components/SproutIllustration.tsx`): the established brand mascot, a chibi baby character with an existing reference sheet (kawaii flat-vector, thin dark-brown linework, cel-shaded pastel, wispy light-brown hair, sage-green trim) already used in the milestone videos and the avatar picker. One asset today, `public/illustrations/hero-sprout-growing.png` (transparent PNG, Sprout at the base of a blooming badge tree) — one pose only, no pose-variant prop yet. Two size variants: `hero` (large Persuade placement, e.g. the homepage two-column hero) and `small` (a supporting detail in an empty or completion state: Library's empty-catalog message and first-run "zero unlocked books" banner, Journey's all-badges-claimed completion moment, Circle's genuine zero-reflections empty state, and `/reviews`' zero-reviews empty state).

**`/login`: no SproutIllustration.** It's the quietest Persuade surface in the app — a single-purpose conversion form (logo, headline, one subhead, then straight into the Google/email buttons), not a page with an empty state or a moment to decorate. Every element already serves getting the visitor through the form fast; adding a mascot here would compete with that goal rather than support it, the same reasoning that excluded Growing's leafless-tree moment above.
